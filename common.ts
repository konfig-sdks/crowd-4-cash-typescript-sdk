...
import { readableStreamToString, Crowd4CashError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new Crowd4CashError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new Crowd4CashError(e, e.response?.data)
                }
                if (innerError instanceof Crowd4CashError) {
                    // Got "Crowd4CashError" from the above try block
                    throw innerError;
                }
